// detectParking.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char** argv)
{
	...
	const unsigned long int total_frames = (unsigned long int)cap.get(cv::CAP_PROP_FRAME_COUNT);
	...
	//Only update sentilo diva every UPDATE_RATIO_PER_S
	clock_t begin_time = clock() - (CLOCKS_PER_SEC * UPDATE_RATIO_PER_S);
	...
	while (cap.isOpened())
	{
		...
		if (DETECT_PARKING)
		{
			for (Parking& park : parking_data)
			{
				...
				cv::Laplacian(roi, laplacian, CV_64F, 5, 2.0, 1.0, 1);
		...
		//Update sentilo every UPDATE_RATIO_PER_S
		if ((float(clock() - begin_time) / CLOCKS_PER_SEC) >= UPDATE_RATIO_PER_S)
		{
			begin_time = clock();
			sentilo.SendData(totalFreeParkings);
		}
